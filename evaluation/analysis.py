...
def average_plot():
    df_interactobot = pd.read_excel('results.xlsx', sheet_name='Average-InteractoBot')
    df_random = pd.read_excel('results.xlsx', sheet_name='Average-Random')
    df_interactobot['Coverage'] = df_interactobot['Coverage'].astype(float)*100
    df_random['Coverage'] = df_random['Coverage'].astype(float)*100

    # Get run columns and convert to percentage
    run_cols_interactobot = [col for col in df_interactobot.columns if col.startswith('Run')]
    run_cols_random = [col for col in df_random.columns if col.startswith('Run')]
    df_interactobot[run_cols_interactobot] = df_interactobot[run_cols_interactobot].astype(float) * 100
    df_random[run_cols_random] = df_random[run_cols_random].astype(float) * 100
    ...
    # Plot confidence bands
    plt.fill_between(df_interactobot["Time"], min_interactobot, max_interactobot,
                     color='slateblue', alpha=0.2)
    plt.fill_between(df_random["Time"], min_random, max_random,
                     color='darkorange', alpha=0.2)

    # Set plot properties
    plt.ylim(0, 100)
    plt.gca().yaxis.set_major_formatter(PercentFormatter())
    plt.xticks(time_points, labels, fontsize=18)
    plt.yticks(fontsize=18)
    plt.grid(axis='y', linestyle='--', alpha=0.6)
    plt.gca().spines['top'].set_visible(False)
    plt.gca().spines['right'].set_visible(False)

    # plt.title('Average XUI Coverage Across All Scenes', fontsize=20, fontweight='bold')
    plt.xlabel('Time (minutes)', fontsize=20)
    plt.ylabel('XUI Coverage (%)', fontsize=20)
    plt.legend(fontsize=18)

    plt.tight_layout()
    plt.savefig('efficiency_average.png', dpi=300, bbox_inches='tight')
    plt.show()
